...
(function($) {
    ...
    if ($.fn.jquery.split('.')[1] < 8) {
        $.expr[':'].Contains = function(a, i, m) {
            return removeDiacritics($(a).text()).toUpperCase().indexOf(m[3].toUpperCase()) >= 0;
        };
    } else {
        $.expr[":"].Contains = jQuery.expr.createPseudo(function(arg) {
            return function( elem ) {
                return removeDiacritics(jQuery(elem).text()).toUpperCase().indexOf(arg.toUpperCase()) >= 0;
            };
        });
    }
...
if (defaultDiacriticsRemovalMap === undefined) {
    /*
     * Remove diacritic function - modified - 6/5/2011
